...
from JS.tools import get_secure_url
from JS.utils import login_user, fetch_cart_items
from JS.config import USERNAME, PASSWORD
...
def main_flow(id_reporttype=13, id_company=203):
    ...
    secure_url_valid = secure_url.replace(
        "auth-service", "report-service"
    )  # adjust URL to the report-service

    # Step 3: Fetch cart items
    return fetch_cart_items(secure_url_valid, id_reporttype, id_company)
